#Drawing Application in Tkinter
...
class Draw():
    def __init__(self,root):
        ...
        #Pick a color for drawing from color palette
        ...
        #Save Button for saving the drawing in local computer
        self.save_btn = Button(self.root,text="Save Drawing",bd=4,bg='white',command=self.save_drawing,width=9,relief=RIDGE)
        self.save_btn.place(x=0,y=257)

        # Background color button for changing the color of the background
        ...
        #Defining a default background color for the Canvas 
        self.background = Canvas(self.root,bg='white',bd=5,relief=GROOVE,height=470,width=680)
        self.background.place(x=80,y=40)
        self.background.bind("<B1-Motion>",self.paint) #Bind the background Canvas with mouse click
...
# Functions are defined here
    ...
    def save_drawing(self):
        try:
            ...
            file_save = filedialog.asksaveasfilename(defaultextension='jpg')
            print(file_save)
            ...
            #print(y1)
            ImageGrab.grab().crop((x , y, x1, y1)).save(file_save)
